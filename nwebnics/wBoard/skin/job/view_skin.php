	<!-- 게시판 상단 출력-->
	<?
	if($board_info[board_head]) echo $board_info[board_head];
	if($_GET[code]=='6_6_1') {
		$diviTitle="구인";
		if(strtotime($view[etc01]) >= strtotime(date('Y-m-d'))) $recruitStatus="<span class=\"statusBtn01\">모집중</span>"; else $recruitStatus="<span class=\"statusBtn03\">모집종료</span>";
	}else {
		if($_GET[code]=='6_6_2') $diviTitle="구직";
	}
	...
	<div class="tblView">
		<table summary="게시물 정보 열람">
			...
			<tbody>
				<tr>
					<th><?=$diviTitle;?>분야</th>
					<td><strong><?=$view[b_class];?></strong></td>
				</tr>
				<tr>
					<th><?=$diviTitle;?>제목</th>
				...
				<tr>
					<th>마감일자</th>
					<td><?=$view[etc01];?> <?=$recruitStatus;?></td>
				</tr>
				<?}?>
				<tr>
					<th>작 성 자</th>
	...
	<div id="aeForm" class="passForm" style="display:none;">
		<form name="aeForm" onsubmit="passCheck(this,'check',''); return false;">
			<div><input type="password" name="passwd" size="10" maxlength="20" class="passbox" /><input type="hidden" name="mode" value="" /><input type="hidden" name="fid" value="<?=$view[fid];?>" /><input type="hidden" name="thread" value="<?=$view[thread];?>" /><input type="hidden" name="mem_id" value="<?=$view[mem_id];?>" /> <input type="image" src="/img/board/delete(btn).gif" width="30" height="20" style="vertical-align:middle;" /> <a href="#endLayer" onclick="formShow('aeForm');"><img src="/img/board/close(btn).gif" style="vertical-align:middle;" /></a></div>
		</form>
	</div>

	<div id="adForm" class="passForm" style="display:none;">
		<form name="adForm" onsubmit="passCheck(this,'delete',''); return false;">
			<div><input type="password" name="passwd" size="10" maxlength="12" class="passbox" /><input type="hidden" name="mode" value="" /><input type="hidden" name="fid" value="<?=$view[fid];?>" /><input type="hidden" name="thread" value="<?=$view[thread];?>" /><input type="hidden" name="mem_id" value="<?=$view[mem_id];?>" /> <input type="image" src="/img/board/delete(btn).gif" width="30" height="20" style="vertical-align:middle;" /> <a href="#endLayer" onclick="formShow('adForm');"><img src="/img/board/close(btn).gif"  style="vertical-align:middle;" /></a></div>
		</form>
	</div>
