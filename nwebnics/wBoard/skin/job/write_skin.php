...
if($_GET[code]=='6_6_1') $diviTitle="구인"; else if($_GET[code]=='6_6_2') $diviTitle="구직";
?>
		...
		<form name="signForm" id="signForm" ENCTYPE="multipart/form-data">
			<fieldset>
				...
				<div id="formWrap">
							...
							<?
								...
								if($board_info[board_class]) {
									...
									for($i=0; $i<count($class_item); $i++) {
										if($view[b_class] == $class_item[$i]) $class_selected=" checked"; else $class_selected="";
										echo "<input type=\"radio\" name=\"b_class\" value=\"".$class_item[$i]."\"".$class_selected." class=\"align_left_middle\" />".$class_item[$i]." ";
									}
								}
							?>
						</dd>
					...
					<dl class="formDl">
						<dt><?=$diviTitle;?>제목</dt>
						<dd><input type="text" name="subject" size="50" maxlength="100" class="textbox textBoxclass" value="<? if(!strcmp($now_path, "reple.php") || !strcmp($now_path, "edit.php")) echo $view[subject];?>" placeholder="<?=$diviTitle;?>제목 (필수)" title="<?=$diviTitle;?>제목 입력" /></dd>
					</dl>
					<?if($_GET[code]=='6_6_1') {?>
					<dl class="formDl">
						<dt>마감일자</dt>
						<dd><input type="text" id="etc01" name="etc01" size="20" class="textbox textBoxclass" maxlength="20" value="<?=$view[etc01];?>" placeholder="마감일 선택" /></dd>
					</dl>
					<?}?>
					<dl class="formDl">
						<dt>작 성 자</dt>
					...
					<dl class="formDl">
						<dt>전화번호</dt>
						<dd><input type="text" name="svc_tel" size="20" maxlength="100" class="textbox" value="<?=$view[svc_tel];?>" title="전화번호 입력" /> 예)02-1234-5678</dd>
					</dl>
					<?}?>
	...
	<div id="boardTail" style="text-align:center; padding:2em 0 0 0;">
		<?
