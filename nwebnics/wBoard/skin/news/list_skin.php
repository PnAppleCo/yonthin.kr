...
<div class="board">
  <div class="board_tab">
		...
		<a href="<?=$_SERVER["PHP_SELF"];?>?code=<?=$_GET[code];?>&b_class=언론보도" class="<?=$cssOn01;?>">언론보도</a>
		<a href="<?=$_SERVER["PHP_SELF"];?>?code=<?=$_GET[code];?>&b_class=보도자료" class="<?=$cssOn02;?>">보도자료</a>
  </div>
  <div class="board_typeA_list">
    <ul>
				<?
				...
				?>
					<li>
						<a href="<?=$link;?>">
							<div class="number"><?=$article_num;?></div>
							<div class="title"><span class="title_type<?=$cssType;?>"><?=$view[$i][b_class]?></span><?=$subject;?></div>
							<div class="date"><?=$view[$i][signdate];?></div>
							<div class="view"><?=$view[$i][click];?></div>
						</a>
					</li>
				<?
				$rTitle = "";//== 제목변수 초기화
