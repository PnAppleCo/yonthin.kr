		...
		<span class="tblRight">
			...
			<?
				...
				if((($board_info[write_level] >= $_SESSION[my_level])) || ($board_info[write_level]==$basicLevel)) {
					$Board_Menu .= "<li><a href=\"javascript:url_move('write','$_GET[code]');\" title=\"쓰기\"><span class=\"boardBtn\">신청</span></a></li>";
				}
		...
		<table summary="게시판 목록">
			...
			<thead>
				<tr>
					...
					<?if($_GET[code]!='5_5') {?>
					<th scope="col"><a href="<?=$_SEVER[PHP_SELF];?>?align_record=subject&align_type=<?=$v_align_type;?>&<?=get_value(1, 'align_record/align_type');?>" class="tbold" title="구분 정렬">구분</a></th>
					<?}?>
			...
			<tbody>
				...
				for($i = $first; $i <= $last; $i++) {
					...
					//== 상담상태 확인
					if($view[$i][svc_status]==0) $btnClass="statusBtn03"; else if($view[$i][svc_status]==1) $btnClass="statusBtn04"; else if($view[$i][svc_status]==2) $btnClass="statusBtn01";
				?>
					<tr>
						<td><?=$article_num;?>.</td>
						<?if($_GET[code]!='5_5') {?>
						...
						<?if($view[$i][secret]>0) {?>
						<div id="acForm<?=$i;?>" class="passForm_List" style="width:160px; display:none;">
						...
						<td><?=$real_email;?></td>
						<td><?=$signdate;?></td>
						<?if($board_info[ps_center]>0) echo "<td><span class=\"".$btnClass."\">".$Arr_psc_status[$view[$i][svc_status]]."</span></td>"; else echo "<td>".$view[$i][click]."</td>";?>
					</tr>
				<?
				$rTitle = "";//== 제목변수 초기화
				$article_num--; }
