...
<div style="background: #f1f1f1;">
	...
	<div class="board">
		<div class="board_tab tab6">
			...
			<a href="<?=$_SERVER["PHP_SELF"];?>?code=<?=$_GET['code'];?>&b_class=뉴스레터" class="<?=$cssOn01;?>">뉴스레터</a>
			<a href="<?=$_SERVER["PHP_SELF"];?>?code=<?=$_GET['code'];?>&b_class=소식지" class="<?=$cssOn02;?>">소식지</a>
		</div>
			...
			<ul>
				...
				for($i = $first; $i <= $last; $i++) {
					...
					if($_GET['keyword']) {
						...
						// PHP82 변환
						// $subject = eregi_replace($_GET['keyword'], "<span style=\"color:#FF6633;\"><strong>$_GET[keyword]</strong></span>", $subject);
						$keyword = preg_quote($_GET['keyword'], '/'); // 정규식 특수문자 이스케이프
						$subject = preg_replace("/$keyword/i", "<span style=\"color:#FF6633;\"><strong>{$_GET['keyword']}</strong></span>", $subject);
					}
					...
					if($board_info['overview'] == 1 && $view[$i]['secret'] != 1) {
						...
						$ucontents = preg_replace("/[[:space:]]+/", " ", $ucontents);
						// PHP82 변환
						// $ucontents = ereg_replace("\"", " ", $ucontents);
						$ucontents = preg_replace('/"/', ' ', $ucontents);
					...
					//== 썸네일 이미지
					$file_ext = explode(".", strrev($view[$i]['filename0']));
