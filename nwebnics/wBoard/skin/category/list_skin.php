		...
		<table class="boardList" summary="게시판 목록">
			...
			<thead>
				<tr>
					...
					<th scope="col"><a href="<?=$_SEVER[PHP_SELF];?>?align_record=name&align_type=<?=$v_align_type;?>&<?=get_value(1, 'align_record/align_type');?>" class="tbold" title="작성자순 정렬">작성자</a></th>
					...
					<?}else {?>
					<th scope="col"><a href="<?=$_SEVER[PHP_SELF];?>?align_record=click&align_type=<?=$v_align_type;?>&<?=get_value(1, 'align_record/align_type');?>" class="tbold" title="조회순 정렬">조회</a></th>
					<?}?>
				</tr>
			</thead>
			<tbody>
				...
				?>
					<tr>
						<td><?=$article_num;?>.</td>
						<td class="ListAlign"><?if($view[$i][b_class]=="보육지원") echo "<span style=\"color:#FF8000;\">[".$view[$i][b_class]."]<span>"; else if($view[$i][b_class]=="육아지원") echo "<span style=\"color:#469CFE;\">[".$view[$i][b_class]."]<span>";?><?=$rTitle;?>
						<?if($view[$i][secret]>0) {?>
						<div id="acForm<?=$i;?>" class="passForm_List" style="width:125px; display:none;">
							...
							<div><input type="password" name="passwd" size="8" maxlength="12" class="passbox" title="비밀번호입력" style="background:url(/nwebnics/img/pass.gif) no-repeat;" onfocus="this.style.backgroundImage=''" onblur="if (this.value=='') this.style.backgroundImage='url(/nwebnics/img/pass.gif)';" /><input type="hidden" name="mode" value="" /><input type="hidden" name="fid" value="<?=$view[fid];?>" /><input type="hidden" name="thread" value="<?=$view[thread];?>" /><input type="hidden" name="mem_id" value="<?=$view[mem_id];?>" /> <input type="image" src="skin/<?=$board_info[skin];?>/img/delete(btn).gif" width="30" height="20" style="vertical-align:middle;" title="확인" /> <a href="#endLayer" onclick="formShow('acForm<?=$i;?>');" title="닫기"><img src="skin/<?=$board_info[skin];?>/img/close(btn).gif" width="30" height="20" style="vertical-align:middle;" alt="닫기" /></a>
							</div>
							</form>
						</span>
						<?}?>
						...
						<td><?=$real_email;?></td>
						<td><?=$signdate;?></td>
						<?if($board_info[ps_center]>0) echo "<td>".$Arr_psc_status[$view[$i][svc_status]]."</td>"; else echo "<td>".$view[$i][click]."</td>";?>
					</tr>
				<?
				$rTitle = "";//== 제목변수 초기화
				$article_num--; }
