		...
		<div class="board_typeC_list">
				...
				for($i = $first; $i <= $last; $i++) {
					...
					if($view[$i][filename0] && $board_info[only_img]>0) {
						...
						$thumbneil_img = "/img/board/400X222.jpg";
					}
					...
					//== 신청 가능 시간 확인
					$startArr=explode("-",$view[$i][etc01]);
					$stopArr=explode("-",$view[$i][etc02]);
				...
				<div>
					<a href="<?=$link;?>">
						...
						<i><?=$appStatus;?></i>
						<p><?=$subject;?></p>
						<span>일정 :<?=strtr($view[$i][etc01],"-",".");?><?if($view[$i][etc02]) echo "~".strtr($view[$i][etc02],"-",".");?></span>
					</a>
				</div>
				<?
				$rTitle = "";//== 제목변수 초기화
